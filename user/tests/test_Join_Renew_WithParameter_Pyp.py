...
class TestJoinPageWithParameterHero:
    ...
    def test_run_test_case(self):
        ...
        if base_url != 'None':
            try:
                ...
                print(f"AN ERROR OCCURRED IN TEST - - JOIN PAGE WITH PARAMETERS Hero: {e}")
            finally:
    ...
    def perform_test_steps(self, driver,url):
        if 'jkc1' in url:
            for i in range(1,3):
                ...
                try:
                    get_by_class_dyn = driver.find_element(By.CLASS_NAME, premium_select)
                except Exception as e:
    ...
    def join_check(self, driver):
        ...
        self._qs_json_application_page = self.common_functions.qs_as_json_object(self._application_page_url)

        self.common_functions.compare_all_qs_params_in_json(self._path, self._application_page_url,
                                                            self._qs_json_lp_page,
                                                            self._qs_json_application_page, self._page_url, 'join')
        time.sleep(2)
        driver.back()
        time.sleep(5)
        if 'jkc1' in json_lp_page:
            get_by_class_dyn = driver.find_element(By.CLASS_NAME, get_by_class_pyp_str[0])
            self.common_functions.button_click(driver, get_by_class_dyn)
        ...
        for i in get_by_class:
            i_href_json = self.common_functions.qs_as_json_object(i.get_attribute('href'))
            i_href_json = json.loads(i_href_json)
            if str(app_json_load['campaignid']).lower() == str(i_href_json['campaignid']).lower():
                continue
            else:
        ...
        time.sleep(5)
        if count>0:
            self.send_mail_cta(self, driver, "join button query string are not matching", self._page_url)
    ...
    def renew_check(self, driver):
        ...
        self.common_functions.compare_all_qs_params_in_json(self._path, self._application_page_url,
                                                            self._qs_json_lp_page,
                                                            self._qs_json_application_page, self._page_url, 'renew')
        time.sleep(2)
        driver.back()
        time.sleep(5)
        if 'jkc1' in json_lp_page:
        ...
        time.sleep(5)
        if count>0:
            self.send_mail_cta(self, driver, "RENEW button query string are not matching", self._page_url)
            print("sent error email")
        else:
