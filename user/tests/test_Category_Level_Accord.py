...
class TestCategoryLevelAccord:
        ...
        if base_url != 'None':
                ...
                try:
                    if 'category' in base_url:
                        accordian_exists = driver.find_elements(By.CLASS_NAME, 'cmp-experiencefragment--accordion')
                        if accordian_exists:
    ...
    def perform_test_steps(self, driver,url):
        ...
        flag=0
        get_by_class_accord = driver.find_elements(By.CLASS_NAME, 'aarp-c-accordion-lp__accTab')
        if json_lp_page:
            for i in get_by_class_accord:
                ...
                if category_qs in str(i.get_attribute('title')).lower().split(" "):
                    ...
                    if 'order: -2;' in i.get_attribute('style') :
                        print(str(json_lp_page['category'][0])+" is set first in the accordian")
                        get_by_class_accord_body = driver.find_elements(By.CLASS_NAME, 'aarp-js-accordion-lp__panel')
                        concat_str='aarp-js-accordion-lp__panel'+str(i.get_attribute('rel'))

                        for x in get_by_class_accord_body:
                            if str(x.get_attribute('id')) == concat_str:
                                if str(x.get_attribute('style')) == 'display: block; order: -1;':
                                    print("Correct accordian is open!")
                                else:
                                    print("accordian closed!")
                                    self.send_mail_cta(self, driver, "category is first but accordian is not open",self._page_url)
