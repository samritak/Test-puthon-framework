...
class TestClick:
    ...
    def perform_test_steps(self, driver, url):
        ...
        for index in width:
            ...
            driver.find_element("tag name", "body").screenshot(
                'Reports/html/screenshot/zoom_img_' + str(index) + "_" + url.split('/')[4] + '.png')
        ...
        #     driver.execute_script(script)
        #     time.sleep(8)
        #     driver.find_element("tag name", "body").screenshot('Reports/html/screenshot/zoom_img_' + str(zoom_size) + '.png')
        #     driver.maximize_window()
        # zoom code ends\
    ...
    def click_join_now_cta(self, driver):
        ...
        for links_value in get_by_class_new:
            ...
            if get_urls.split('?')[0] != final_after_link.split('?')[0]:
                ...
                self.send_mail_cta(self, driver, "Join Button CTA is not matching", get_urls)
                elements_not_clickable = driver.find_elements(By.CSS_SELECTOR, ".aarp-js-lp--joinNow[style*='pointer-events']")
                for element_not_clickable in elements_not_clickable:
                    if element_not_clickable:
                        ...
                        self.send_mail_cta(self, driver, "Join Button CTA is not clickable", not_clickable_link)
    ...
    def click_renew_now_cta(self, driver):
        ...
        for links_value in get_by_class_new:
            ...
            if get_urls.split('?')[0] != final_after_link.split('?')[0]:
                print("the url is not matching : ", get_urls)
                self.send_mail_cta(self, driver, "Renew Button CTA is not matching", get_urls)
                elements_not_clickable = driver.find_elements(By.CSS_SELECTOR, ".aarp-js-lp--renew[style*='pointer-events']")
                for element_not_clickable in elements_not_clickable:
                    if element_not_clickable:
                        not_clickable_link = element_not_clickable.get_attribute('href')
                        print("Renew Button CTA is not clickable", not_clickable_link)
                        self.send_mail_cta(self, driver, "Renew Button CTA is not clickable", not_clickable_link)
