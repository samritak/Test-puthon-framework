...
class TestJoinPageWithoutParameter(BaseTemplate):
    ...
    def perform_test_steps(self):
        ...
        if self.exp_name_match == "sem":
            self.join_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_SEM_JOIN_CAMPAIGN_ID,
                                             self._campaign_id)
        elif self.exp_name_match == "soc":
            self.join_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_SOC_JOIN_CAMPAIGN_ID,
                                             self._campaign_id)
        elif self.exp_name_match == "dis":
            self.join_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_DIS_JOIN_CAMPAIGN_ID,
                                             self._campaign_id)
        else:
            self.join_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_JOIN_CAMPAIGN_ID,
                                             self._campaign_id)
