    ...
    def perform_test_steps(self, url):
        ...
        for index in width:
            ...
            try:
                ...
                driver.set_window_size(self.full_page_screenshot_url('Width'),
                                        self.full_page_screenshot_url('Height'))
                driver.find_element("tag name", "body").screenshot(
                "Reports/html/screenshot/page_load_screenshot_" + url.split('/')[4] + "_" + str(index) + ".png")
                time.sleep(3)
            except Exception as e:
