...
class TestCTAQueryString:
    ...
    def perform_test_steps(self, driver,url):
        ...
        get_by_join_class_accord = driver.find_elements(By.CLASS_NAME, 'aarp-js-lp--joinNow')
        get_by_renew_class_accord = driver.find_elements(By.CLASS_NAME, 'aarp-js-lp--renew')
        get_by_rejoin_class_accord = driver.find_elements(By.CLASS_NAME, 'aarp-js-lp--rejoinNow')
        join_count=0
        renew_count=0
        rejoin_count=0
        if json_lp_page:
            if json_lp_page['cta'][0] == 'j':
                for i in get_by_join_class_accord:
                    ...
                    if parent_div.tag_name == 'div' and parent_div.value_of_css_property('display') != 'none':
                        if 'join' in  str(i.get_attribute("Title")).split(' ')[0].lower():
                            continue
                        else:
                            self.send_mail_cta(self, driver, "Wrong button Title in CTA",self._page_url)
                            print(str(i.get_attribute("Title")) + "is the title, email sent!")
                    else:
                        div_link=parent_div.find_element(By.XPATH, "..")
                        if div_link.value_of_css_property('display') != 'none' and str(dataformelementid).lower() != 'none':
                            join_count += 1
                if join_count > 0:
                    self.send_mail_cta(self, driver, "Join Button does not exist or hidden", self._page_url)
                ...
                for i in get_by_renew_class_accord:
                    ...
                    parent_div = i.find_element(By.XPATH,"..")
                    if parent_div.tag_name == 'div' and parent_div.value_of_css_property('display') != 'none':
                        if 'renew' in str(i.get_attribute("Title")).split(' ')[0].lower():
                        ...
                        if div_link.value_of_css_property('display') != 'none' : #and str(dataformelementid).lower() != 'none':
                            renew_count += 1
                if renew_count > 0:
                    self.send_mail_cta(self, driver, "Renew Button does not exist or hidden", self._page_url)
                ...
                for i in get_by_rejoin_class_accord:
                    ...
                    #dataformelementid = i.get_attribute('data-formelementid')
                    if parent_div.tag_name == 'div' and parent_div.value_of_css_property('display') != 'none':
                        if 'rejoin' in str(i.get_attribute("Title")).split(' ')[0].lower():
                        ...
                        if div_link.value_of_css_property('display') != 'none': # and str(dataformelementid).lower() != 'none' :
                            rejoin_count += 1
                if rejoin_count > 0:
                    self.send_mail_cta(self, driver, "Rejoin Button does not exist or hidden", self._page_url)
