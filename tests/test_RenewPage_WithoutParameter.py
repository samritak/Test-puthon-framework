...
from RenewPage_WithoutParameter import RenewPage
from user.Config.config import Global_Env_Data
import csv


class TestRenewPageWithoutParameter(BaseTemplate):
    def test_renew(self):
        self.contents = []
        path_to_file = Global_Env_Data.LOCAL_PATH + Global_Env_Data.TEMPLATE_WITHOUT_PARAMETER
    ...
    def perform_test_steps(self):
        ...
        self._campaign_id = self.renew_page.qs_value_from_name(self._findme_page_url, 'campaignid')

        self._tc_hpc = self.renew_page.qs_value_from_name(self._findme_page_url, 'tc_hpc')

        self._tc_tm_version = self.renew_page.qs_value_from_name(self._findme_page_url, 'tc_tm_version')

        self.exp_name_match = self.renew_page.search_name(self._path)

        if self.exp_name_match == "sem":
            self.renew_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_SEM_RENEW_CAMPAIGN_ID,
                                              self._campaign_id)
        elif self.exp_name_match == "soc":
            self.renew_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_SOC_RENEW_CAMPAIGN_ID,
                                              self._campaign_id)
        elif self.exp_name_match == "dis":
            self.renew_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_DIS_RENEW_CAMPAIGN_ID,
                                              self._campaign_id)
        else:
            self.renew_page.compare_qs_values("campaignid", Global_Env_Data.DEFAULT_RENEW_CAMPAIGN_ID,
                                              self._campaign_id)
