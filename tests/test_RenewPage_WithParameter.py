...
class TestRenewPageWithParameter:
    ...
    def perform_test_steps(self, driver):
        ...
        time.sleep(6)
        self.full_page_screenshot_app = lambda x: driver.execute_script('return document.body.parentNode.scroll' + x)
        driver.set_window_size(self.full_page_screenshot_app('Width'),
        self.full_page_screenshot_app('Height'))  # May need manual adjustment
        driver.find_element("tag name", "body").screenshot('Reports/html/screenshot/application_page.png')
        self._qs_json_findme_page = self.common_functions.qs_as_json_object(self._findme_page_url)
        self.common_functions.compare_all_qs_params_in_json(self._path, self._findme_page_url, self._qs_json_lp_page,
                                                            self._qs_json_findme_page, self._page_url, 'renew')
